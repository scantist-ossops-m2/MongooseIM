...
run_ct_with_report(Groups) ->
    ct:run_test([{suite, "tests/test_SUITE"}, {group, Groups}, {ct_hooks, [cth_surefire]}]),
    ReportPath = string:strip(os:cmd("ls -t ct_run.*/junit_report.xml | head -1"), right, $\n),
...
expected_skipped_group_report() ->
    FullGroup = <<"deeply_nested_test_group.nested_test_group.skipped_test_group">>,
    skip_all(expected_group_report(FullGroup)).

skip_all(Report) ->
    [{Test, Group, skipped} || {Test, Group, _} <- lists:flatten(Report)].

expected_group_report() ->
    FullGroup = <<"deeply_nested_test_group.nested_test_group.test_group">>,
    expected_group_report(FullGroup).

expected_group_report(FullGroup) ->
    [{<<"init_per_group">>, FullGroup, ok},
     test_report(FullGroup),
     {<<"end_per_group">>, FullGroup, ok}].

test_report(FullGroup) ->
    [{<<"passing_tc">>, FullGroup, ok},
     {<<"skipped_tc">>, FullGroup, skipped},
     {<<"failing_tc_1">>, FullGroup, failed},
     {<<"failing_tc_2">>, FullGroup, failed}, % init_per_testcase failed
     {<<"failing_tc_3">>, FullGroup, ok} % end_per_testcase failed, still ok
    ].
